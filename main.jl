...
@time begin
    for timestep in rationalTimeSteps
        ...
        dir = "images\\$(Int.(c))_pi2_stepfunc\\rational_real"
        if !isdir(dir)
            mkpath(dir)
        end
...
@time begin
    for timestep in rationalTimeSteps
        ...
        dir = "images\\$(Int.(c))_pi2_stepfunc\\rational_img"
        if !isdir(dir)
            mkpath(dir)
        end

        if timestep == 0
            filename = "$(dir)\\$(Int.(c))_pi2_0_1000_00005pi_imaginary.png"
        else
            filename = "$(dir)\\$(Int.(c))_pi2_$(numerator(timestep))pi$(denominator(timestep))_1000_00005pi_imaginary.png"
        end
        savefig(filename)
    end
end
...
@time begin
    for timestep in irrationalTimeSteps
        ...
        dir = "images\\$(Int.(c))_pi2_stepfunc\\irrational_real"
        if !isdir(dir)
            mkpath(dir)
        end

        filename = "$(dir)\\$(Int.(c))_pi2_$(timestep)_1000_00005pi_real.png"
        savefig(filename)
    end
end
...
@time begin
    for timestep in irrationalTimeSteps
        ...
        dir = "images\\$(Int.(c))_pi2_stepfunc\\irrational_img"
        if !isdir(dir)
            mkpath(dir)
        end

        filename = "$(dir)\\$(Int.(c))_pi2_$(timestep)_1000_00005pi_imaginary.png"
        savefig(filename)
    end
end